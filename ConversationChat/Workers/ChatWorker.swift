...
class ChatWorker {
    ...
    func prepopulateContactsIntoRealm() {
        ...
        try! realm.write {
            for contact in contacts {
                ...
                if realmContact.id == 1 {
                    UserDefaultsConstant.CurrentUser.setValue(value: realmContact)
                }
            }
        }
    }
    ...
    func createChatRoom(withContacts  contacts: [Contact], completionHandler completion: @escaping (ChatRoom) -> Void)
    {
        ...
        try! realm.write {
            ...
            chatRoom.id = UUID().hashValue
            chatRoom.room_description = "Friends"
            chatRoom.timestamp = Date()
            chatRoom.unreadMesagesCount = 0
            chatRoom.logoUrl = "https://cdn.jwplayer.com/thumbs/32UR6cye-720.jpg"
            ...
            completion(ChatRoom(withRealmChatRoom: chatRoom))
        }
    }
}
